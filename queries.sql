...
insert into Company(ID, name, City, phone_number, email, employer_name, Sector) values (6,'Company-F', 21, '+90507 624 8798', 'company-F@gmail.com', 'Poyraz Yılmaz',4);
...
insert into Company(ID, name, City, phone_number, email, employer_name, Sector) values (15,'Company-O', 4, '+44507 897 4324', 'company-O@gmail.com', 'Ahmet Akın',5);
...
insert into Company(ID, name, City, phone_number, email, employer_name, Sector) values (17,'Company-S', 16, '+90530 780 9080', 'company-S@gmail.com', 'Pınar Eren',1);
...
-- ####################################################################################################
 ...
 /* List the companies that self cover the insurance cost and whose ship crashed during transportation*/
select Company.name
from Company, Cruise_Detail, Cruise, Includes
where Company.ID = Cruise_Detail.Company
and Cruise_Detail.ID = Includes.Cruise_Detail_ID
and Cruise.ID = Includes.Cruise_ID
and Cruise_Detail.Insurance_Cost = 'Yes'
and Cruise.Ship_Wreck = 'Yes';

/* List the establisment years of container shipping companies whose warehouses are in Istanbul*/
select Company_Shipping.Foundation_Date
from Company_Shipping, City
where Company_Shipping.City = City.ID
and City.name = 'Istanbul';

-- ####################################################################################################
-- Query
-- ####################################################################################################

/* finds all container_count that is greater than minimum container_count */
select container_count
from cruise_detail  
where container_count > all (select min(container_count)
                             from cruise_detail);
                            
/* finds attributes of sector and company that are in company table and orders them in descending order */
SELECT*
FROM Sector s
RIGHT OUTER JOIN company c ON s.ID=c.sector 
ORDER BY s.ID DESC;

/*Join Company_Shipping, Company and Cruise Tables */
SELECT*
FROM Company_Shipping
JOIN Company ON Company_Shipping.ID=Company.ID 
JOIN Cruise  on Company.ID=Cruise.ID;

/* average capacity of the non- accident ships */
select avg(s.capacity)  
from ship s, cruise c, related_to rt 
where s.id = rt.ship_id and rt.cruise_id = c.id and c.ship_wreck = 'No';


/* Show the departure place and departure date of the voyages whose arrival date or fullness is unknown */
select id, from_city, departure_date 
from cruise 
where is_full is null or arrival_date is null;


/* find all companies given in both not paid the insurance cost and their goods have arrived safely */
select distinct  c.name
from company c, cruise_detail cd 
where c.id = cd.company and cd.insurance_cost = 'No' and exists (select *
                                                                   from cruise c2, cruise_detail cd2, includes i
                                                                   where c2.ship_wreck = 'No' and 
                                                                   cd2.id = i.cruise_detail_id 
                                                                   and c2.id = i.cruise_detail_id);
                                                       
/* find the ships with capacities between 1500 and 3000 and its name not starts with B then order by their names */                                                                
select name, capacity
from ship 
where capacity between 1500 and 3000 and name not like 'B%'
order by name ;
  
/* list the voyages of ships with both full capacity and no accident */
(select * 
from cruise
where is_full = 'Yes') intersect (select * from cruise where ship_wreck = 'No');

/*Sum of the capacities of ships with IDs greater than 5
*/
select SUM(Capacity)
FROM Ship
Where Ship.ID>5;

/*List and order all values that have no associated Company_Shipping with a lower ID than 3 according to descending */
SELECT *
FROM Country as C
WHERE NOT EXISTS(SELECT*
                 From Company_Shipping AS CS
                 WHERE C.ID=CS.ID AND CS.ID<3
                 )
ORDER BY ID DESC;

/*List all values according to cruise with ID greater than 6 but did not crash */
(SELECT*
FROM Cruise
WHERE Cruise.ID > 6)
EXCEPT
(Select*
FROM Cruise
Where Cruise.Ship_Wreck = 'Yes');

/* List all employees whose sector number is not an odd number */
Select employer_name
From Company  
Where sector NOT In (1,3,5,7);


/* minimum capacity vessel making voyage */
select name, capacity
from ship
where capacity = (
            select min(capacity)
 ...
 /* maximum capacity vessel making voyage */
select name, capacity
from ship
...
/* find city names and country names whose country ids are 2, 8, 13*/
select distinct city.name, country.name
from city, country
where city.country in (2, 8, 13) and city.country = country.id;

 
/* list the ids and names of the ships that have sailed but did not crash and whose name start with 'C'*/
select *
from (select s.id, s.name
      from ship s, cruise c, related_to rt 
      where s.id = rt.ship_id and rt.cruise_id = c.id and c.ship_wreck = 'No') a
where a.name like 'C%';

/* list the number of voyages of ships that did not crash */
select s.id, s.name, 
       (select count(*) from related_to r where r.ship_id =s.id) count_cruise
from ship s, cruise c, related_to rt 
where s.id = rt.ship_id and rt.cruise_id = c.id and c.ship_wreck = 'No';

/* list the IDs of the voyages and company names with the container count greater than 2500 */
select c.id, c.name
from cruise_detail cd, company c
where cd.company = c.id and cd.id = some (select id
                                                       from cruise_detail 
                                                       where container_count > 2500); 
                                                    
/* find cruise details that container count is equal to 1250 or 6000 */         
(select * from cruise_detail
where  container_count = 1250)
union 
(select  * from cruise_detail
where container_count= 6000);
